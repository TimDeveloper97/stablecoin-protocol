...
contract Treasury is ITreasury, Operator, ReentrancyGuard {
    ...
    function initialize(uint256 _startTime, uint256 _epoch_length)
        external
        onlyOperator
    {
    ...
    function globalCollateralValue() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pools_array.length; i++) {
            ...
            if (pools_array[i] != address(0)) {
                total_collateral_value = total_collateral_value.add(
                    IPool(pools_array[i]).collateralDollarBalance()
                );
            }
        }
    ...
    function calcEffectiveCollateralRatio() public view returns (uint256) {
        ...
        uint256 ecr = total_collateral_value.mul(PRICE_PRECISION).div(
            total_supply_dollar
        );
    ...
    function refreshCollateralRatio() public {
        ...
        if (current_dollar_price > price_target.add(price_band)) {
            ...
            if (target_collateral_ratio <= ratio_step) {
                ...
                target_collateral_ratio = target_collateral_ratio.sub(
                    ratio_step
                );
            }
        }
        // IRON price is below $1 - `price_band`. Need to increase `collateral_ratio`
            ...
            if (
                ...
                target_collateral_ratio = target_collateral_ratio.add(
                    ratio_step
                );
            }
        }
    ...
    {
        uint256 total_collateral_value = globalCollateralValue();
        uint256 target_collateral_value = IERC20(dollar)
            .totalSupply()
            .mul(target_collateral_ratio)
            .div(PRICE_PRECISION);
        if (total_collateral_value >= target_collateral_value) {
            _collateral_value = total_collateral_value.sub(
                target_collateral_value
            );
            _exceeded = true;
        } else {
            _collateral_value = target_collateral_value.sub(
    ...
    function _swap(
        ...
        require(
            vswap_router != address(0) &&
                vswap_pair_share_bnb != address(0) &&
                vswap_pair_bnb_busd != address(0),
            "!vswap"
        );
        ...
        uint256[] memory out_amounts = IValueLiquidRouter(vswap_router)
        ...
        checkEpoch
    {
        ...
        if (_exceeded) {
            _allocation_value = _excess_collateral_value
                .mul(excess_collateral_distributed_ratio)
                .div(RATIO_PRECISION);
            uint256 collateral_price = IPool(rebalancing_pool)
                .getCollateralPrice();
            uint256 _allocation_amount = _allocation_value
                .mul(PRICE_PRECISION)
    ...
    function restoreCollateralRatios(
        uint256 _target_collateral_ratio,
        uint256 _effective_collateral_ratio
    ) external onlyOperator {
        require(
            _target_collateral_ratio <= COLLATERAL_RATIO_MAX &&
                _effective_collateral_ratio <= COLLATERAL_RATIO_MAX,
            "invalidRatio"
        );
    ...
    function setRebalanceCooldown(uint256 _rebalance_cooldown)
        public
        onlyOperator
    {
    ...
    function setExcessDistributionRatio(
        uint256 _excess_collateral_distributed_ratio
    ) public onlyStrategistOrOperator {
    ...
    function executeTransaction(
        ...
        require(
            success,
            string(
                "Treasury::executeTransaction: Transaction execution reverted."
            )
        );
